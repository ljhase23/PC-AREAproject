          ...
          <table class="list-table">
            <thead>
              ...
              // board테이블에서 idx를 기준으로 내림차순해서 10개까지 표시
                $sql = query("select * from pcarea_notice order by notice_number desc limit 0,10"); 
                  while($pcarea_notice = $sql->fetch_array())
                  {
                    //title변수에 DB에서 가져온 title을 선택
                    $title=$pcarea_notice["notice_title"]; 
                    if(strlen($title)>30)
                    { 
                      //title이 30을 넘어서면 ...표시
                      $title=str_replace($pcarea_notice["notice_title"],mb_substr($pcarea_notice["notice_title"],0,30,"utf-8")."...",$pcarea_notice["notice_title"]);
                    }
              ?>
            <tbody>
              <tr>
                <td width="70"><?php echo $pcarea_notice['notice_number']; ?></td>
                <td width="500"><a href="read.php?notice_number=<?php echo $pcarea_notice['notice_number']?>"><?php echo $title;?></a></td>
                <td width="120"><?php echo $pcarea_notice['notice_writer']?></td>
                <td width="100"><?php echo $pcarea_notice['date']?></td>
                <td width="100"><?php echo $pcarea_notice['notice_view']; ?></td>
                <!-- 추천수 표시해주기 위해 추가한 부분 -->
                <td width="100"><?php echo $pcarea_notice['notice_suggest']?></td>
              </tr>
            </tbody>
            <?php } ?>
          </table>
